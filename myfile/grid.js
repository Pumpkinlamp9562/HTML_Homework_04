...
var mapSketch = function(p5j){
  ...
  $('#lbutton').click(()=>{
    let url = 'https://spreadsheets.google.com/feeds/list/1aSq1KlCngLEB0YCdlKrTXlHCSyFBWbMMuBqhMPzDwSE/1/public/values?alt=json';
        console.log(url);
        p5j.httpGet(url, 'jsonp', false, function(response) {
  ...
  $('#ubutton').click(()=>{
    if (p5j.arrarea.length>0) {
        ...
        let exeurl = 'https://script.google.com/macros/s/AKfycbz0T2ZQKi3IZZ9Ddy9i-u2_6iUJAYdW-GTpN9UWJMS1OKrJuXw/exec';
        
        let editurl = 'https://docs.google.com/spreadsheets/d/1wFKqmwgI3-uvlw2_YcZoBYMWou07_Ohkr8sMZWnlHV0/edit#gid=0';
        $.post(exeurl,{
  ...
  $('#ibutton').click(()=>{
    let url = 'https://spreadsheets.google.com/feeds/list/1wFKqmwgI3-uvlw2_YcZoBYMWou07_Ohkr8sMZWnlHV0/1/public/values?alt=json';
        console.log(url);
        p5j.httpGet(url, 'jsonp', false, function(response) {
          p5j.download = response; // 會把所有回呼資料存於 earthquakes
        });
